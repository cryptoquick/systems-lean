...
present-partial, open, SYSTEMS_LEAN_HOST
...
Host JOIN-ALG ConsumeToken axioms live in `SystemsLean/Linear.lean` +
HostCompose mint/consume. ErasedIndex / UnrestrictedShare are **cite inputs**
for the matrix dual row -- not reimplemented under `src/systems/`.
...
|-----|--------|---------------|-------|
| Multiplicity surface (MULT-0 / MULT-1 / MULT-OMEGA) | **present-partial** | `SystemsLean/Mult.lean` | Closed inductive; FAIL-CLOSED-UNKNOWN-GRADE on raw tags |
| Linear / JOIN-ALG duals (ConsumeToken + ErasedIndex / UnrestrictedShare cite) | **present-partial** | Linear.lean + JoinMap dual cite + SurfaceMatrix dualCiteOk | Classic elaborator cannot enforce MULT-1; axioms + live-flag model |
| Typed IR / ordered program / graph edges | **present-partial** | Types + IrProgram + IrGraph | CAP 8 / EDGE_MAX 16; List vs C arrays PARTIAL |
...
| Join map | **present-partial** | JoinMap.lean HOST-JOIN-MAP / SLAKE_JOIN_MAP_V0 | Duals read-only; not formal full bridge theorems |
...
- PARTIAL host inventory: `host-partial-inventory.md` (Mult..ProductPath; 31 modules; HOST-INVENTORY-CLOSE + HOST-PRODUCT-PATH readiness; intentional PARTIAL carry)
- Freestanding self-host acceptance: `self-host.md` (SH0; Mult kernel IR SH1;
  ...
  ProductPath productPathReady (not residual free; not product complete); not
  freestanding product self-host complete; not llvm unlocked;
  not PROVABLY)
